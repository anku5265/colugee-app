  ...
  const fetchAttendance = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('attendance')
        .select(`
          id, attendance_date, status, student_id,
          schedules:schedule_id (subject, title)
        `)
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p>No attendance records for {dateFilter}</p>
          </div>
        ) : (
              ...
              {records.map((record) => (
                <tr key={record.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">
                    {(record.schedules as any)?.subject || (record.schedules as any)?.title || '—'}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {record.profiles?.department || '—'}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${statusColor(record.status)}`}>
